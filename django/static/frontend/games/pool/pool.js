...
function updateTurnMessage() {
    ...
    const message = player1 !== player2 && playerLocked === 1 ?  
...
function myfunc() {
    ...
    if (checkWin('X')) {
        document.getElementById('print').innerHTML = `${player1 !== player2  && playerLocked === 1 ? player1 : "X's"} has won`;
        sendWinnerMessage(player1, player2, 'Tic-Tac-Toe');
        buttons.forEach(btn => btn.disabled = true);
        buttons.forEach((btn, index) => { if (board[index] === 'X') btn.style.color = "red"; });
    } else if (checkWin('O')) {
        document.getElementById('print').innerHTML = `${player1 !== player2 && playerLocked === 1 ? player2 : "O's"} has won`;
